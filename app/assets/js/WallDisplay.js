...
/**
 * "Nice numbers" Achsenskala (Heckbert-Algorithmus): rundet Min/Max/Schritt
 * auf glatte Werte (1/2/5 * 10^n), damit Beschriftungen wie "20 Grad" statt
 * krummer Zwischenwerte wie "17.9 Grad" erscheinen.
 */
...
function niceTicks(min, max, count = 5) {
    ...
    const ticks   = [];
    for (let v = niceMin; v <= niceMax + step / 1e6; v += step) ticks.push(+v.toFixed(6));
...
class WallChart {
    ...
    _drawComparison() {
        ...
        const { min: yMin, max: yMax, ticks } = niceTicks(rawMin - yPad, rawMax + yPad, 6);
    ...
    _draw() {
        ...
        const xMin = Math.min(...xs), xMax = Math.max(...xs);
        const rawMin = Math.min(...ys), rawMax = Math.max(...ys);
        const yPad = (rawMax - rawMin) * 0.15 || 1;
        const { min: yMin, max: yMax, ticks } = niceTicks(rawMin - yPad, rawMax + yPad, 6);
        this._geom = { padL, padR, padT, padB, innerW, innerH, xMin, xMax, yMin, yMax, w, h };
